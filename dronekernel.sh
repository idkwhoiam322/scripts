...
if [[ "$@" =~ "stable"* ]]; then 
	...
	export ZIPNAME="${KERNEL_BUILD_TYPE}-r${DRONE_BUILD_NUMBER}-$(git rev-parse --abbrev-ref HEAD).$(grep "SUBLEVEL =" < Makefile | awk '{print $3}')$(grep "EXTRAVERSION =" < Makefile | awk '{print $3}').zip"
fi
...
if [[ "$@" =~ "gcc" ]]; then
	...
	make -j${KEBABS} O=out ARCH=arm64 CC="/drone/src/clang/clang-r370808/bin/clang" CLANG_TRIPLE="aarch64-linux-gnu-" CROSS_COMPILE="/drone/src/gcc/bin/aarch64-linux-android-" CROSS_COMPILE_ARM32="/drone/src/gcc32/bin/arm-linux-androideabi-"
fi
END=$(date +"%s")
DIFF=$((END - START))
